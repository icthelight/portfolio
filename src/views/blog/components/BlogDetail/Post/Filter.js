    ...
    return (
        <Container>
            <Desc>
              ...
              <p>
                먼저, 필터 아이템을 클릭했을때 필터 아이템의 하단에 팝업으로 드롭 박스가 뜨도록 만들어보자.
              </p>
              <Image src={filter1}/>
              ...
              <p>그리고, useState를 활용하여 필터아이템을 클릭했을때 open인 상태가 되고, open 이면 드롭박스를 보여주도록 설정하면 된다.</p>
              <p>이제 드롭박스에 나온 드롭메뉴아이템을 클릭하면 해당 페이지로 라우팅되도록 설정해보자.</p>
              <Image src={filter3}/>
              <p>
                위 url 이 라우팅되면 페이지가 어떤 검색 화면을 보여줄지 설정해야 한다. 현재 비동기 통신은 searchContainer 컴포넌트에서 하고 있다.
